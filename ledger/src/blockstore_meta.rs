pub use crate::index_v2::IndexV2;
use {
    crate::{
        bit_array::{self},
        blockstore::MAX_DATA_SHREDS_PER_SLOT,
        index_v2::ShredIndexV2,
...
impl TryFrom<IndexLegacy> for IndexV2 {
    type Error = bit_array::Error;

    fn try_from(index: IndexLegacy) -> Result<Self, Self::Error> {
        Ok(IndexV2 {
            slot: index.slot,
            data: index.data.try_into()?,
            coding: index.coding.try_into()?,
        })
    }
}
...
impl IndexLegacy {
    ...
    pub fn data(&self) -> &ShredIndexLegacy {
        &self.data
    }
    pub fn coding(&self) -> &ShredIndexLegacy {
        &self.coding
    }

    pub(crate) fn data_mut(&mut self) -> &mut ShredIndexLegacy {
        &mut self.data
    }
    pub(crate) fn coding_mut(&mut self) -> &mut ShredIndexLegacy {
        &mut self.coding
    }
}
...
impl ShredIndexLegacy {
    ...
    pub(crate) fn count_ones<R>(&self, bounds: R) -> usize
    where
        R: RangeBounds<u64>,
    {
...
impl ShredIndexV2 {
    ...
    #[inline]
    fn convert_bounds<R>(bounds: R) -> (Bound<usize>, Bound<usize>)
    where
        R: RangeBounds<u64>,
    {
        (
            bounds.start_bound().map(|&i| i as usize),
            bounds.end_bound().map(|&i| i as usize),
        )
    }

    #[cfg(test)]
    pub fn range<R>(&self, bounds: R) -> impl Iterator<Item = usize> + '_
    where
        R: RangeBounds<u64>,
    {
        self.index
            .range_unchecked(Self::convert_bounds(bounds))
            .indices()
    }

    pub fn count_ones<R>(&self, bounds: R) -> usize
    where
        R: RangeBounds<u64>,
    {
        self.index
    ...
    pub fn remove(&mut self, index: u64) {
        if let Ok(true) = self.index.remove(index as usize) {
            self.num_shreds -= 1;
        }
    }
}

impl TryFrom<ShredIndexLegacy> for ShredIndexV2 {
    type Error = bit_array::Error;

    fn try_from(value: ShredIndexLegacy) -> Result<Self, Self::Error> {
        if value.index.len() > MAX_DATA_SHREDS_PER_SLOT {
            return Err(bit_array::Error::OutOfBounds);
        }
        let mut index = ShredIndexV2 {
            num_shreds: value.index.len(),
            ..Default::default()
        };
        for i in value.index {
            index.index.insert_unchecked(i as usize);
        }
        Ok(index)
    }
}
...
impl From<ShredIndexV2> for ShredIndexLegacy {
    fn from(value: ShredIndexV2) -> Self {
        ShredIndexLegacy {
            index: value.index.indices().map(|i| i as u64).collect(),
        }
    }
}
...
impl ErasureMeta {
    ...
    pub(crate) fn status(&self, index: &Index) -> ErasureMetaStatus {
        ...
        let num_coding = index.coding().count_ones(self.coding_shreds_indices());
        let num_data = index.data().count_ones(self.data_shreds_indices());
...
mod test {
    ...
    proptest! {
        ...
        fn shred_index_legacy_compat(
            ...
            assert_eq!(
                v2.range(range.clone()).sum::<usize>(),
                legacy.range(range).sum::<u64>() as usize
            );

            assert_eq!(ShredIndexV2::try_from(legacy.clone()), Ok(v2));
